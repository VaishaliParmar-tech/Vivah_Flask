...
def admin_update_custom_order():
    ...
    order.status = request.form.get('status'); db.session.commit()
...
def forgot_password():
    if request.method == 'POST':
        user = User.query.filter_by(email=request.form.get('email','').strip().lower()).first()
        if user:
            token = secrets.token_urlsafe(32); user.reset_token = token; db.session.commit()
            link = url_for('reset_password', token=token, _external=True)
            flash(f'Reset link: <a href="{link}" style="color:var(--gold)">Click here</a>', 'success')
        else: flash('If that email exists, a link has been sent.', 'info')
...
def reset_password(token):
    ...
    if request.method == 'POST':
        pw = request.form.get('password','')
        if pw != request.form.get('password2',''): flash('Passwords do not match.', 'error'); return redirect(url_for('reset_password', token=token))
        user.set_password(pw); user.reset_token = None; db.session.commit()
...
def cart():
    ...
    for item in cart_items:
        ...
        if p: cart_products.append({'product': p, 'qty': item['qty']}); subtotal += p.price * item['qty']
...
def add_to_cart(id):
    ...
    for item in cart:
        if item['id'] == id: item['qty'] += 1; session['cart'] = cart; return jsonify({'success': True, 'message': 'Added to cart!'})
    cart.append({'id': id, 'qty': 1}); session['cart'] = cart
...
def checkout():
    ...
    if request.method == 'POST':
        ...
        items_data = [{'name': i['product'].name, 'price': i['product'].price, 'qty': i['qty']} for i in cart_products]
        order = Order(order_number=gen_order_number(), user_id=user.id, total=final_total,
            discount_amount=discount, coupon_code=coupon_code, status='Confirmed',
            address=f"{request.form.get('address')}, {request.form.get('city')}, {request.form.get('state')} - {request.form.get('pincode')}",
            payment_method=request.form.get('payment_method','COD'), items_json=json.dumps(items_data))
    ...
    with app.app_context():
        ...
        if Category.query.count() == 0:
            cats = [
                ...
                ('Kanjivaram Sarees','kanjivaram-sarees','Authentic South Indian silk','categories/kanjivaram_1.jpeg'),
                ...
                ('Bridal Collection','bridal-sarees','Exquisite bridal sarees','categories/bride_4.jpeg'),
